...
protected:
    ...
    void setClumpVelocity(int index, double3 velocity)
    {
        if (index >= hos.clumps.num) return;
        if (simPara.iStep > 0)
        {
            dev.clumps.uploadState(hos.clumps);
            hos.clumps.dyn.velocities[index] = velocity;
    ...
    void addDummyParticleWall(std::vector<double3> p, double spacing);

    void addSolid(std::vector<double3> p, double3 velocity, double radius, double density, int materialID);

    void addInfiniteWall(double3 p, double3 normal, int materialID);

    void addCylinderWall(double3 p, double3 axis, double3 angularVelocity, double radius, int materialID);

	void addCluster(std::vector<double3> p, std::vector<double> radius, std::vector<double> density, double3 velocity, int materialID);

    void addClump(std::vector<double3> p, std::vector<double> radius, double3 centroidPosition, double3 velocity, double3 angularVelocity, double mass, symMatrix inertiaTensor, int materialID);
    ...
    const HostFluid& getHostFluidData()
    {
