...
void solverBase::outputFluid2SolidVTU()
{
    if (simPara.iFrame > 0)
    {
    ...
    MKDIR(dir.c_str());
    std::ostringstream fname;
    fname << dir << "/fluid2Solid_" << std::setw(4) << std::setfill('0') << simPara.iFrame << ".vtu";
    std::ofstream out(fname.str().c_str());
    if (!out) throw std::runtime_error("Cannot open " + fname.str());
    out << std::fixed << std::setprecision(10);
    const int N = hos.fluid2Solid.num;
    out << "<?xml version=\"1.0\"?>\n"
        "<VTKFile type=\"UnstructuredGrid\" version=\"0.1\" byte_order=\"LittleEndian\">\n"
        "  <UnstructuredGrid>\n";
    ...
    out << "      <Points>\n"
        "        <DataArray type=\"Float32\" NumberOfComponents=\"3\" format=\"ascii\">\n";
    for (int i = 0; i < N; ++i) {
        const int& idx_i = hos.fluid2Solid.objectPointed[i];
        const int& idx_j = hos.fluid2Solid.objectPointing[i];
        double3 p = 0.5 * (hos.fluids.points.position[idx_i] + hos.solids.points.position[idx_j]);
        out << ' ' << p.x << ' ' << p.y << ' ' << p.z;
    }
    out << "\n        </DataArray>\n"
        "      </Points>\n";
    ...
    out << "      <PointData Scalars=\"force\">\n";

    const struct {
        const char* name;
        const std::vector<double3>& vec;
    } vec3s[] = {
        { "force"       , hos.fluid2Solid.force        }
    };
    for (size_t k = 0; k < sizeof(vec3s) / sizeof(vec3s[0]); ++k) {
        out << "        <DataArray type=\"Float32\" Name=\"" << vec3s[k].name
            << "\" NumberOfComponents=\"3\" format=\"ascii\">\n";
