	...
	int iStep)
{
	...
	double V0_i = pow(h_i / 1.3, 3);
	double sumW = wendlandKernel3D(0, h_i);
	double rho_star = f.rho0[idx_i] + f.dRho[idx_i];
	...
	for (int k = idx_i > 0 ? f.fluidNeighbor.prefixSum[idx_i - 1] : 0; k < f.fluidNeighbor.prefixSum[idx_i]; k++)
	{
		...
		sumW += wendlandKernel3D(length(r_i - r_j), h_i);
	}
	...
	for (int k = idx_i > 0 ? f.solidNeighbor.prefixSum[idx_i - 1] : 0; k < f.solidNeighbor.prefixSum[idx_i]; k++)
	{
		...
		double V0_a = pow(2. * s.radius[idx_a], 3);
		sumW += wendlandKernel3D(length(r_i - r_a), h_i) * V0_a / V0_i;
	}
	...
	{
		double rho1 = fmax(rho_star, f.rho0[idx_i] * sumW / f.sumW0[idx_i]);
		f.dRho[idx_i] = rho1 - f.rho0[idx_i];
	}
}
	...
	double dt)
{
	...
	for (int k = idx_i > 0 ? f.solidNeighbor.prefixSum[idx_i - 1] : 0; k < f.solidNeighbor.prefixSum[idx_i]; k++)
	{
		...
		double h_a = 2.6 * s.radius[idx_a];

		double h = 0.5 * (h_i + h_a);
		double c = c_i;
		...
		double3 v_star = (P_i - P_a) / c / (rho_i + rho_a) * (-e_ia) + (rho_i * v_i + rho_a * (2 * v_a - v_i)) / (rho_i + rho_a);
...
__global__ void calDummyParticlePressure(solid s,
	fluid f,
	interactionBase fluid2Solid,
	double3 g)
{
...
void fluidIntegrate(DeviceData& d, double timeStep, int iStep, int integrateGap, int maxThreadsPerBlock)
{
	...
	double timeStep_f = timeStep * double(integrateGap);

	if (iStep % (5 * integrateGap) == 0 || iStep == 1) densityReinitialization << <grid, block >> > (d.fluids, d.solids, d.fluid2Fluid, d.fluid2Solid, iStep);
...
__global__ void calInfiniteWallForceTorque(solid s, interactionSolid2InfiniteWall solid2InfiniteWall, 
	...
	double dt)
{
	int idx_i = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx_i >= s.points.num) return;
	if (s.isDummyParticle[idx_i] == 1 || s.inverseMass[idx_i] < 1.e-10) return;
	...
	for (int k = 0; k < infiniteWalls.num; k++)
	{
		double3 r_j = infiniteWalls.pointInPlane[k];
		double3 n_w = infiniteWalls.normal[k];
		double3 n_ij = n_w;
		double3 r_ij = r_i - r_j;
		double delta = rad_i - dot(r_ij, n_w);
		if (delta < 0.)
		{
			solid2InfiniteWall.slidingSpring[idx_i * infiniteWalls.num + k] = make_double3(0, 0, 0);
			solid2InfiniteWall.rollingSpring[idx_i * infiniteWalls.num + k] = make_double3(0, 0, 0);
			solid2InfiniteWall.torsionSpring[idx_i * infiniteWalls.num + k] = make_double3(0, 0, 0);
			return;
		}
		double rad_ij = rad_i;
		double m_ij = 1. / s.inverseMass[idx_i];
		double3 r_c = r_i - (rad_i - delta) * n_ij;
		double3 v_c_ij = v_i + cross(w_i, r_c - r_i);
		double3 w_ij = w_i;
		...
		int mat_i = s.materialID[idx_i];
		int mat_j = infiniteWalls.materialID[k];
		int c_ij = contactModels.getCombinedIndex(mat_i, mat_j);

		double3 epsilon_s = solid2InfiniteWall.slidingSpring[idx_i * infiniteWalls.num + k];
		double3 epsilon_r = solid2InfiniteWall.rollingSpring[idx_i * infiniteWalls.num + k];
		double3 epsilon_t = solid2InfiniteWall.torsionSpring[idx_i * infiniteWalls.num + k];
		...
		solid2InfiniteWall.slidingSpring[idx_i * infiniteWalls.num + k] = epsilon_s;
		solid2InfiniteWall.rollingSpring[idx_i * infiniteWalls.num + k] = epsilon_r;
		solid2InfiniteWall.torsionSpring[idx_i * infiniteWalls.num + k] = epsilon_t;

		s.dyn.accelerations[idx_i] += F_c * s.inverseMass[idx_i];
		s.torques[idx_i] += T_c + cross(r_c - r_i, F_c);
	}
}

__global__ void calCylinderWallForceTorque(solid s, interactionSolid2InfiniteWall solid2CylinderWall,
	...
	double dt)
{
	...
	for (int k = 0; k < cylinderWalls.num; k++)
	{
		double rad_j = cylinderWalls.radius[k];
		double3 asix = cylinderWalls.axis[k];
		double3 r_j = cylinderWalls.pointInAxis[k];
		double3 r_ij = r_i - r_j;
		...
		if (delta >= rad_i)
		{
			delta = rad_i + rad_j - length(r_i - r_c0);
		}
		if (delta < 0.)
		{
			solid2CylinderWall.slidingSpring[idx_i * cylinderWalls.num + k] = make_double3(0, 0, 0);
			solid2CylinderWall.rollingSpring[idx_i * cylinderWalls.num + k] = make_double3(0, 0, 0);
		...
		double3 v_c_ij = v_i + cross(w_i, r_c - r_i) - (cross(w_j, r_c - r_j));
		double3 w_ij = w_i - w_j;
		...
		int mat_i = s.materialID[idx_i];
		int mat_j = cylinderWalls.materialID[k];
		int c_ij = contactModels.getCombinedIndex(mat_i, mat_j);

		double3 epsilon_s = solid2CylinderWall.slidingSpring[idx_i * cylinderWalls.num + k];
		double3 epsilon_r = solid2CylinderWall.rollingSpring[idx_i * cylinderWalls.num + k];
		double3 epsilon_t = solid2CylinderWall.torsionSpring[idx_i * cylinderWalls.num + k];
		...
		solid2CylinderWall.slidingSpring[idx_i * cylinderWalls.num + k] = epsilon_s;
		solid2CylinderWall.rollingSpring[idx_i * cylinderWalls.num + k] = epsilon_r;
		solid2CylinderWall.torsionSpring[idx_i * cylinderWalls.num + k] = epsilon_t;
	...
	double3 g)
{
	...
	for (int i = clumps.pebbleStartIndex[idx_c]; i < clumps.pebbleEndIndex[idx_c]; i++)
	{
		...
		double3 F_i = make_double3(0, 0, 0);
		if (s.inverseMass[i] > 1.e-10) F_i = s.dyn.accelerations[i] / s.inverseMass[i];
...
void calSolidContactAfterFluidIntegrate(DeviceData& d, double timeStep, int maxThreadsPerBlock)
{
	...
	if (d.infiniteWalls.num > 0) calInfiniteWallForceTorque << <grid, block >> > (d.solids, d.solid2InfiniteWall, d.infiniteWalls, d.contactModels, timeStep);

	if (d.cylinderWalls.num > 0) calCylinderWallForceTorque << <grid, block >> > (d.solids, d.solid2CylinderWall, d.cylinderWalls, d.contactModels, timeStep);
