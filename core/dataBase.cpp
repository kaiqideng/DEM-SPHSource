...
void dataBase::addDummyParticleWall(std::vector<double3> p, double radius)
{
...
void dataBase::addInfiniteWall(double3 p, double3 normal, int materialID)
{
    if (simPara.iStep > 0)
    {
        std::cout << "Error: infinite wall can only be added before the simulation starts." << std::endl;
        return;
    }
    HostInfiniteWall i(1);
    i.pointInPlane[0] = p;
    i.normal[0] = normal;
    i.materialID[0] = materialID;
    hos.infiniteWalls.insertData(i);
}

void dataBase::addCylinderWall(double3 p, double3 axis, double3 angularVelocity, double radius, int materialID)
{
    if (simPara.iStep > 0)
    {
        std::cout << "Error: cylinder wall can only be added before the simulation starts." << std::endl;
        return;
    }
    HostCylinderWall c(1);
    c.pointInAxis[0] = p;
    c.axis[0] = axis;
    c.angularVelocity[0] = angularVelocity;
    c.materialID[0] = materialID;
    hos.cylinderWalls.insertData(c);
}
...
void dataBase::addClump(std::vector<double3> p, std::vector<double> radius, double3 centroidPosition, double3 velocity, double3 angularVelocity, double mass, symMatrix inertiaTensor, int materialID)
{
    ...
    for (size_t i = 0; i < p.size();i++)
    {
        ...
        s.materialID[i] = materialID;
        s.clumpID[i] = clumpID;
        if (density > 0. && physicalRadius > 0) s.inverseMass[i] = 1. / (pow(2. * physicalRadius, 3) * density);
    }
...
void dataBase::addFluidData(const HostFluid f)
{
    if (simPara.iStep > 0)
    {
        ...
        hos.fluid2Solid.insertData(HostInteractionBase(20 * f.points.num));
...
void dataBase::addSolidData(const HostSolid s)
{
    if (simPara.iStep > 0)
    {
        ...
        setSpatialGrids();
        dev.solid2InfiniteWall.build(dev.infiniteWalls.num, dev.solids.points.num);
        dev.solid2CylinderWall.build(dev.cylinderWalls.num, dev.solids.points.num);
    }
    else
    {
        hos.solids.insertData(s);
...
void dataBase::buildDeviceData()
{
    ...
    dev.solids.copy(hos.solids);
    dev.clumps.copy(hos.clumps);
    dev.infiniteWalls.copy(hos.infiniteWalls);
    dev.fluid2Fluid.copy(hos.fluid2Fluid);
    dev.fluid2Solid.copy(hos.fluid2Solid);
    dev.solid2Solid.copy(hos.solid2Solid);
    dev.solid2InfiniteWall.build(dev.infiniteWalls.num, dev.solids.points.num);
