...
struct HostSolid
{
	...
	std::vector<int> isDummyParticle;
	...
	explicit HostSolid(int n)
		...
		torques(n, make_double3(0, 0, 0)), angularVelocities(n, make_double3(0, 0, 0)), radius(n, 0), inverseMass(n, 0), materialID(n, 0), clusterID(n, -1), clumpID(n, -1), isDummyParticle(n, 0), pressure(n, 0)
	{
	}
	...
	void insertData(const HostSolid s)
	{
		...
		isDummyParticle.insert(isDummyParticle.end(), s.isDummyParticle.begin(), s.isDummyParticle.end());
...
struct HostInfiniteWall
{
	int num{ 0 };
	std::vector<double3> pointInPlane;
	std::vector<double3> normal;
	std::vector<int> materialID;

	HostInfiniteWall() = default;
	explicit HostInfiniteWall(int n)
		: num(n), pointInPlane(n, make_double3(0., 0., 0.)), normal(n, make_double3(0., 0., 0.)), materialID(n, 0)
	{
	}

	void insertData(const HostInfiniteWall i)
	{
		pointInPlane.insert(pointInPlane.end(), i.pointInPlane.begin(), i.pointInPlane.end());
		normal.insert(normal.end(), i.normal.begin(), i.normal.end());
		materialID.insert(materialID.end(), i.materialID.begin(), i.materialID.end());
		num += i.num;
	}
};

struct HostCylinderWall
{
	int num{ 0 };
	std::vector<double3> pointInAxis;
	std::vector<double3> axis;
	std::vector<double3> angularVelocity;
	std::vector<double> radius;
	std::vector<int> materialID;

	HostCylinderWall() = default;
	explicit HostCylinderWall(int n)
		: num(n), pointInAxis(n, make_double3(0., 0., 0.)), axis(n, make_double3(0., 0., 0.)), angularVelocity(n, make_double3(0., 0., 0.)), radius(n, 0.), materialID(n, 0)
	{
	}

	void insertData(const HostCylinderWall c)
	{
		pointInAxis.insert(pointInAxis.end(), c.pointInAxis.begin(), c.pointInAxis.end());
		axis.insert(axis.end(), c.axis.begin(), c.axis.end());
		angularVelocity.insert(angularVelocity.end(), c.angularVelocity.begin(), c.angularVelocity.end());
		radius.insert(radius.end(), c.radius.begin(), c.radius.end());
		materialID.insert(materialID.end(), c.materialID.begin(), c.materialID.end());
		num += c.num;
	}
};
...
struct HostData
{
	...
	HostSolid solids;
	HostClump clumps;
	HostInfiniteWall infiniteWalls;
	HostCylinderWall cylinderWalls;
