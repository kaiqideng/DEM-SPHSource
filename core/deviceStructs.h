...
struct solid
{
	...
	void alloc(int n)
	{
		...
		CUDA_ALLOC(isDummyParticle, n, InitMode::ZERO);
	...
	void copy(const HostSolid& s)
	{
		...
		cuda_copy(isDummyParticle, s.isDummyParticle.data(), nCopy, CopyDir::H2D);
...
struct infiniteWall
{
	...
	{
		num = n;
		CUDA_ALLOC(pointInPlane, n, InitMode::ZERO);
		CUDA_ALLOC(normal, n, InitMode::ZERO);
		CUDA_ALLOC(materialID, n, InitMode::ZERO);
	}
	...
	void release()
	{
		CUDA_FREE(pointInPlane);
		CUDA_FREE(normal);
		CUDA_FREE(materialID);
		num = 0;
	}

	void copy(const HostInfiniteWall& i)
	{
		release();
		int nCopy = i.num;
		alloc(nCopy);
		cuda_copy(pointInPlane, i.pointInPlane.data(), nCopy, CopyDir::H2D);
		cuda_copy(normal, i.normal.data(), nCopy, CopyDir::H2D);
		cuda_copy(materialID, i.materialID.data(), nCopy, CopyDir::H2D);
	}
};
...
{
	int num{ 0 };
	double3* pointInAxis{ nullptr };
	double3* axis{ nullptr };
	...
	{
		num = n;
		CUDA_ALLOC(pointInAxis, n, InitMode::ZERO);
		CUDA_ALLOC(axis, n, InitMode::ZERO);
		CUDA_ALLOC(angularVelocity, n, InitMode::ZERO);
		CUDA_ALLOC(radius, n, InitMode::ZERO);
		CUDA_ALLOC(materialID, n, InitMode::ZERO);
	}
	...
	void copy(const HostCylinderWall& c)
	{
		release();
		int nCopy = c.num;
		alloc(nCopy);
		cuda_copy(pointInAxis, c.pointInAxis.data(), nCopy, CopyDir::H2D);
		cuda_copy(axis, c.axis.data(), nCopy, CopyDir::H2D);
		cuda_copy(angularVelocity, c.angularVelocity.data(), nCopy, CopyDir::H2D);
		cuda_copy(radius, c.radius.data(), nCopy, CopyDir::H2D);
		cuda_copy(materialID, c.materialID.data(), nCopy, CopyDir::H2D);
	}
};
...
struct interactionSolid2InfiniteWall
{
	int cap{ 0 };
	...
	{
		cap = n;
		CUDA_ALLOC(slidingSpring, cap, InitMode::ZERO);
		CUDA_ALLOC(rollingSpring, cap, InitMode::ZERO);
		CUDA_ALLOC(torsionSpring, cap, InitMode::ZERO);
	}
	...
	void build(int numInfiniteWalls, int numSolids)
	{
		if (cap == 0)
		{
			alloc(numInfiniteWalls * numSolids);
		}
		else
		{
			int newCap = numInfiniteWalls * numSolids;
			std::vector<double3> spring1(newCap, make_double3(0, 0, 0)), spring2(newCap, make_double3(0, 0, 0)), spring3(newCap, make_double3(0, 0, 0));
			if (newCap > cap)
			{
				cuda_copy(spring1.data(), slidingSpring, cap, CopyDir::D2H);
				cuda_copy(spring2.data(), rollingSpring, cap, CopyDir::D2H);
				cuda_copy(spring3.data(), torsionSpring, cap, CopyDir::D2H);
				release();
				alloc(newCap);
				cuda_copy(slidingSpring, spring1.data(), cap, CopyDir::H2D);
				cuda_copy(rollingSpring, spring2.data(), cap, CopyDir::H2D);
				cuda_copy(torsionSpring, spring3.data(), cap, CopyDir::H2D);
			}
		}
	}
};
...
struct DeviceData
{
	...
	interactionSolid2InfiniteWall solid2InfiniteWall;
	interactionSolid2InfiniteWall solid2CylinderWall;
	...
	void release()
	{
		...
		solids.release();
		clumps.release();
		infiniteWalls.release();
		cylinderWalls.release();
		fluid2Fluid.release();
		fluid2Solid.release();
		solid2Solid.release();
		solid2InfiniteWall.release();
		solid2CylinderWall.release();
